 ...
 */
function valideMail($email){return (bool)preg_match('/^[a-z0-9%_+.-]+@(([a-z0-9][a-z0-9-]{0,62}(?<!-)\.)*([a-z]{2,4}|museum)|(([0-9]|[1-9][0-9]|(1[0-9][0-9]|2([0-4][0-9]|5[0-5])))\.){3}([0-9]|[1-9][0-9]|(1[0-9][0-9]|2([0-4][0-9]|5[0-5])))(?<!0\.0\.0\.0))$/i', $email)? $email: false;}
 ...
 */
function is_IP($IP){return (bool)preg_match('/(([0-9]|[1-9][0-9]|(1[0-9][0-9]|2([0-4][0-9]|5[0-5])))\.){3}([0-9]|[1-9][0-9]|(1[0-9][0-9]|2([0-4][0-9]|5[0-5])))(?<!0\.0\.0\.0)/', $IP);}
 ...
 */
function is_Hex($Hex, $len = 0){return (bool)preg_match('/[a-fA-F0-9]'.($len == 0? '*': "{{$len}}").'/i', $Hex)? $Hex: false;}

/**
 *	Verifies input as numeric
 *	@param Num		Mixed	The var to validate
 *	@return			Mixed	$Num if numeric; FALSE on error/invalid
 */
function is_number($Num){return (bool)preg_match('/([0-9]+)/i', $Num)? $Num: false;}
 ...
 *	@return			1 On INFECTED; We found an Exploit!
 *					0 On CLEAN; The data is ok to pass to the database
 */
function check_inject ($check = array()) {
...
#	debug_check();
	$meta = preg_grep('/(\=|;|\'|"|--) * (%27|%23|%3B|%6F|%4F|%72|%52|%3D) * ^(\d\d\d\d\d) * ^(\d\d\d\d\d\d\d\d\d\d) + [(%(\d\s))] + [????(2-7)]/ix',$check);
	if (count($meta) >= 1) {
		$f = fOpen($log);
		fWrite($f, 
