<?php
/**
 *	@Name: ~/includes/library/Kizano/Misc.php
 *	@Date: 2010-07-19
 *	@Depends: None
 *	@Description: Contains miscelaneous functions and stuff to execute.
 *	@Notes: Edit with care
 *	
 *	Kizano Zend-friendly library
...
class Kizano_Strings{
	...
	public static function hashPass($plaintext, $salt){
		return hash('sha256', $salt.hash('haval256,5', $salt.$plaintext.hash('tiger160,4', $salt.$plaintext).$salt).$salt);
	}
	...
	public static function backtrace(){
		...
		foreach($debug as $i => $deb){
			...
			foreach($deb['args'] as $k => $d){
				is_object($d) && $debug[$i]['args'][$k] = '(object)'.get_class($d);
			}
		}
		return($debug);
	}
